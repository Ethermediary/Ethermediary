...
window.onload = function () {
  ...
  mutual_cancel_vendeur_button = document.getElementById(
    "mutual_cancel_vendeur"
  );
  mutual_cancel_acheteur_button = document.getElementById(
    "mutual_cancel_acheteur"
  );
  refuse_cancel_acheteur_button = document.getElementById(
    "refuse_cancel_acheteur"
  );
  refuse_cancel_vendeur_button = document.getElementById(
    "refuse_cancel_vendeur"
  );
  acheteur_cancel_offer_button = document.getElementById(
    "acheteur_cancel_offer"
  );
...
function ACHETEUR_makeOffer(amount) {
  ...
  if (state == STATE_INIT) {
    if (balance_acheteur < offerAmount + caution) {
      ...
      vendeur_screen =
        "an offer for: " + (offerAmount - caution) + " was made, accept ?";
...
function ACHETEUR_receivedPackage() {
  if (state == STATE_WAITING_SEND) {
    ...
    vendeur_screen =
      "ACHETEUR received your package, here is a proof:[pic] please tell us next time you sent it";
...
function ACHETEUR_receivedBadPackage() {
  if (state == STATE_WAITING_SEND) {
    ...
    vendeur_screen =
      "ACHETEUR didn't like your product, he is sending it back to you, please tell us when you sent it next time";
  } else if (state == STATE_WAITING_ARRIVE) {
    state = STATE_SENDING_BACK_BAD_PACKAGE;

    acheteur_screen = "please send back the item and provide proof";
    vendeur_screen =
      "ACHETEUR didn't like your product, he is sending it back to you";
  }
  updateDisplay();
}
...
function ACHETEUR_sendBackProof() {
  if (state == STATE_SENDING_BACK_BAD_PACKAGE) {
    ...
    acheteur_screen =
      "thanks! you will get your money back when VENDEUR received the package";
    vendeur_screen =
      "ACHETEUR sent back the package here is the proof: [pic], tell us when you reiceived it";
  }
  updateDisplay();
}
...
function VENDEUR_acceptOffer() {
  ...
  if (state == STATE_OFFER_MADE) {
    ...
    acheteur_screen =
      "waiting for VENDEUR to send your package, if you already received it tell us";
...
function VENDEUR_provideProof() {
  if (state == STATE_WAITING_SEND) {
    ...
    acheteur_screen =
      "VENDEUR sent the package, here is the proof:[pic] please tell us when you receive it to get your money back";
    vendeur_screen =
      "thank you ! you will get paid when ACHETEUR will receive the package";
  }
  updateDisplay();
}
...
function VENDEUR_receivedBadPackage() {
  if (state == STATE_SENT_BACK_BAD_PACKAGE) {
    ...
    acheteur_screen =
      "you package arrived! here is your money back, tell us when you sent it next time";
    vendeur_screen = "thanks! here is your money back";
  }
  updateDisplay();
}
